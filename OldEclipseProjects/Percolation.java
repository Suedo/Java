...
//	private static int count = 0;
	...
	/*
	 * all methods of WeightedQuickUnionFind are linear array based.
	 * however, here the the array (N*N) is 2-D , and array calculations are based on [1 to N] naming convention.
	 * hence to make them inter-compatible , we use the toLinear method.
	 * 
	 */
	...
	private void setup(int N){
		side = N;
		wqu = new WeightedQuickUnion(N*N + 2); // N*N + virtual top + virtual bottom cells
	...
	private void checkDirectionAndCallUnionOn(Point p){
		if(northOK(p)) connectNorth(p);
		if(southOK(p)) connectSouth(p);
		if(westOK(p)) connectWest(p);
		if(eastOK(p)) connectEast(p);
	}
...
//				count++;
	...
	public boolean isFull(int row , int col){ // is the site full ?
		return wqu.connected(0, toLinear(row, col));
	}
